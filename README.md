...
Alternatively, you might wish to write using an erasable ink (Such as my biodegradable wet erasable fountain pen ink! See my Instructables article on how to prepare it at home: https://www.instructables.com/Recipe-for-a-Multipurpose-Biodegradable-and-Wet-Er/) on notebook pages printed on transparencies with a laser printer. You could then pass in "scrptreader_acetate:" as an additional argument when running <b>PrintANotebook</b>, with the same parameters explained in the paragraph above. The pages would then be mirrored, such that you would print them on the left-hand half of a transparency sheet for the first half of your PDF document pages, then flip the stack of transparencies in your printer and print the second half of your PDF document pages on the reverse side (opposite to those you already printed, but on the other side of the transparency). There is also a vertical line indicating where you will need to cut the transparency in half in order to assemble your reusable notebook. The default heading on the pages will be "Write on this side" which guides you in remembering on which side of the transparency to write, in order to avoid scratching the laser toner on the reverse side. You can change the heading to any other text, preceded by the argument "heading_text_right:" when running the Python code.  
