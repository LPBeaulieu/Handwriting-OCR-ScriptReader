...
JPEG_file_names = ([file_name for file_name in sorted(os.listdir(os.path.join(cwd, "Training&Validation Data"))) if file_name[-4:] == ".jpg"])
...
gutter_margin_width_pixels = 0.75*3300/8.5
lines_between_text = 3
...
if len(sys.argv) > 1:
    ...
    try:
        for j in range(1, len(sys.argv)):
            if len(sys.argv[j]) > 1 and sys.argv[j][:13] == "top_left_dot:":
                top_left_dot = True
                top_left_dot_x = int(sys.argv[j][13:].split(",")[0])
                top_left_dot_y = int(sys.argv[j][13:].split(",")[1])
            elif len(sys.argv[j]) > 1 and sys.argv[j][:14] == "top_right_dot:":
                top_right_dot = True
                top_right_dot_x = int(sys.argv[j][14:].split(",")[0])
                top_right_dot_y = int(sys.argv[j][14:].split(",")[1])
            elif len(sys.argv[j]) > 1 and sys.argv[j][:12] == "dot_spacing:":
                inches_between_dots = float(sys.argv[j][12:].strip())
            elif len(sys.argv[j]) > 1 and sys.argv[j][:10] == "x_overlap:":
                x_overlap = int(sys.argv[j][10:].strip())
            elif len(sys.argv[j]) > 1 and sys.argv[j][:10] == "y_overlap:":
                y_overlap = int(sys.argv[j][10:].strip())
            elif len(sys.argv[j]) > 1 and sys.argv[j].lower()[:11] == "top_margin:":
                inches = float(sys.argv[j][11:].strip())
                top_margin_y_pixel = round(inches*2550/8.5)
            elif len(sys.argv[j]) > 1 and sys.argv[j].lower()[:14] == "bottom_margin:":
                inches = float(sys.argv[j][14:].strip())
                bottom_margin_y_pixel = 2550-round(inches*2550/8.5)
            elif len(sys.argv[j]) > 1 and sys.argv[j].lower()[:12] == "left_margin:":
                inches = float(sys.argv[i][12:].strip())
                left_margin_x_pixel = round(inches*3300/8.5)
            elif len(sys.argv[j]) > 1 and sys.argv[j].lower()[:13] == "right_margin:":
                inches = float(sys.argv[i][13:].strip())
            elif len(sys.argv[j]) > 1 and sys.argv[j].lower()[:14] == "gutter_margin:":
                gutter_margin_width_pixels = round(float(sys.argv[i].lower()[14:].strip())*3300/8.5)
            elif len(sys.argv[j]) > 1 and sys.argv[j].lower()[:19] == "lines_between_text:":
                lines_between_text = int(sys.argv[j].lower()[19:].strip())
                print("lines_between_text: ", lines_between_text)


    except Exception as e:
        print(e)
        print('Please provide the top left dot x,y coordinates for the first scanned page '
        + '(in pixels, but without units), preceded by "top_left_dot:", ' +
        ' as well as those of the top right dot, after '
        + '"top_right_dot:", as well as the ' +
        'dot spacing, in inches, but without units, after "dot_spacing:".')
        print('For example: "top_left_dot:542,295" top_right_dot:2042,302: "dot_spacing:0.1"')
...
#The slope of the line connecting the top two corner dots is calculated and will
...
#in a similar way.
slope = (top_right_dot_y - top_left_dot_y)/(top_right_dot_x-top_left_dot_x)
...
pixels_between_dots = round(inches_between_dots*2550/8.5)
#If the user didn't provide a number of pixels for the horizontal overlap of
#every character segmentation cropped image, it is defaulted to a sixth of
#the "pixels_between_dots".
if x_overlap == None:
    x_overlap = round(pixels_between_dots/6)
...
if y_overlap == None:
    y_overlap = round(0.40*lines_between_text*pixels_between_dots)
...
with alive_bar(len(JPEG_file_names)) as bar:
    for i in range(len(JPEG_file_names)):
        ...
        #The "get_next_x(current_x, k)" function takes in two parameters, namely the
        #current "x" coordinate on a given line ("current_x"), and the character
        #index on that line ("k"). This function will determine the next "x"
        #coordinate of the next character on the same line, by adding the
        #the current "x" coordinate ("current_x") to the difference between
        #the next_line's starting "x" coordinate in the untilted page
        #("dot_x_coordinates[k+1]") and the first line's starting "x"
        #coordinate in the untilted page ("dot_x_coordinates[0]"),
        #multiplied by the cosine of the tilt angle ("np.cos(slope_angle)").
        def get_next_x(current_x, k):
        ...
        #Similarly, the "y" coordinate of the next character on the same line is
        #determined by adding the current "y" coordinate to the difference between
        #the current "x" coordinate ("current_x") to the difference between
        #the next_line's starting "x" coordinate in the untilted page
        #("dot_x_coordinates[k+1]") and the first line's starting "x"
        #coordinate in the untilted page ("dot_x_coordinates[0]"),
        #multiplied by the sine of the tilt angle ("np.cos(slope_angle)").
        def get_next_y(current_y, k):
            next_y = round(current_y + ((dot_x_coordinates[k+1]-dot_x_coordinates[0])*np.sin(slope_angle)))
            return next_y
        ...
        #coordinates, distance in-between dots ("inches_between_dots") as well as
        #top and bottom margins ("top_margin_y_pixel" and "bottom_margin_y_pixel",
        #respectively) that were used to generate the actual dot grid notebook pages.
        ...
        #respectively.
        dot_x_coordinates = get_dot_x_coordinates(inches_between_dots, 5)
        dot_y_coordinates = get_dot_y_coordinates(inches_between_dots, 5)
        ...
        text_line_numbers = [0]
        #The "current_line" counter is initialized to 1 and will keep track of the line
        #number, in order to determine if the line under investigation is a line that
        #will undergo segmentation or a spacer in-between lines of text.
        current_line = 1
        ...
        for j in range(len(dot_y_coordinates)):
            #If the "current_line" number is equal to the previous line number
            #("text_line_numbers[-1]") where there was text, plus the number of
            #empty lines in-between lines of text, plus 1 to bring it even to
            #the current_line number, then this line will undergo segmentation
            #and the "current_line" number will be stored in the list "text_line_numbers"
            if current_line == text_line_numbers[-1] + lines_between_text + 1:
                text_line_numbers.append(current_line)
            current_line += 1
        ...
        for j in range(len(text_line_numbers)):
            ...
            #to che current line in the untilted page), after being multiplied by the sine of the scanned page tilt angle.
            #This result is then subtracted from the "top_left_dot_x". A similar calculation is performed
            #to determine "next_line_y", but this time with the cosine of the slope angle.
            if slope > 0:
                next_line_x = round(top_left_dot_x - (dot_y_coordinates[text_line_numbers[j]]-dot_y_coordinates[text_line_numbers[0]])*np.sin(slope_angle))
                next_line_y = round(top_left_dot_y + (dot_y_coordinates[text_line_numbers[j]]-dot_y_coordinates[text_line_numbers[0]])*np.cos(slope_angle))
                ...
                y_threshold = round(top_left_dot_y + (dot_y_coordinates[text_line_numbers[-1]]-dot_y_coordinates[text_line_numbers[0]])*np.cos(slope_angle)) + y_overlap
            elif slope < 0:
                next_line_x = round(top_left_dot_x - (dot_y_coordinates[text_line_numbers[j]]-dot_y_coordinates[text_line_numbers[0]])*np.sin(slope_angle))
                next_line_y = round(top_left_dot_y + (dot_y_coordinates[text_line_numbers[j]]-dot_y_coordinates[text_line_numbers[0]])*np.cos(slope_angle))
                #If the slope is negative (the image is tilted counter-clockwise, given that y coordinates increase going down,
                #then the maximal "x" coordinate will be greater than "top_right_dot_x", so the horizontal distance is determined
                #using a right angle triangle with a hypothenuse equal to the total height of the dot grid in the untilted page).
                x_threshold = round(top_right_dot_x - (dot_y_coordinates[text_line_numbers[-1]]-dot_y_coordinates[text_line_numbers[0]])*np.sin(slope_angle) + x_overlap)
                y_threshold = round(top_right_dot_y + (dot_y_coordinates[text_line_numbers[-1]]-dot_y_coordinates[text_line_numbers[0]])*np.cos(slope_angle)) + y_overlap
            #If the scanned page is untilted (well done!), the "next_line_x" would line up exactly with the "top_left_dot"
            #the "next_line_y" would be the the cumulative vertical distance from the origin up to che current line in the
            #untilted page, which doesn't need to be corrected by trigonometric calculations in this case, and adding this
            #result to the top left corner "y" coordinate.
            elif slope == 0:
                next_line_x = top_left_dot_x
                next_line_y = round(top_left_dot_y + (dot_y_coordinates[text_line_numbers[j]]-dot_y_coordinates[text_line_numbers[0]]))
                #As the page isn't tilted, all of the right coordinates will line up to the top right dot "x" coordinate,
                #with the addition of the "x_overlap".
                x_threshold = top_right_dot_x + x_overlap
                #The "y_threshold" is simply the total height of the dot grid, added to the top_right_dot_y (the top_left_dot_y
                #would have done just as well.)
                y_threshold = top_right_dot_y + (dot_y_coordinates[text_line_numbers[-1]]-dot_y_coordinates[text_line_numbers[0]])
            ...
            for k in range(len(dot_x_coordinates)-1):
                ...
                next_x = get_next_x(current_x,k)
                next_y = get_next_y(current_y, k)
                #If the "next_x" is lower than the "x_threshold" and the "next_y" is inferior to
                #both the "y_threshold" and the last dot "y" coordinate in the untilted dot grid
                #("dot_y_coordinates[-1]"), then it is included in the "chars_x_y_coordinates" list
                #at the current "j" line index. The more stringent threshold requirements for the
                #"y" axis (with the addition of "dot_y_coordinates[-1]") is to avoid segmenting
                #another line well below what should be allowed. "pixels_between_dots" needs to
                #be added to the "next_y", in order to bring the coordinate to the lower right corner
                #of the rectangle.
                if next_x < x_threshold and next_y < y_threshold and next_y < dot_y_coordinates[-1]:
                    ...
                    (cv2.rectangle(text_image_copy, (chars_x_y_coordinates[j][k][0][0], chars_x_y_coordinates[j][k][0][1]),
                    (chars_x_y_coordinates[j][k][1][0], chars_x_y_coordinates[j][k][1][1]), (0,255,0),3))
